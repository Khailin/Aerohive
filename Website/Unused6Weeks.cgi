...
my $query1 = qq{select Username from LastLogin where (STR_TO_DATE(replace(DATE,'/',','),'%d,%m,%Y') <= NOW() - INTERVAL 6 WEEK or Date IS NULL) and Revoked=false ORDER BY Username};
